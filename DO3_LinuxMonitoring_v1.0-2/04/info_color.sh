...
arr1[0]=$column1_background
arr1[1]=$column1_font_color
arr1[2]=$column2_background
arr1[3]=$column2_font_color
...
# Соотносим переменные с названиями цветов
for i in 0 1 2 3
do
if [[ ${arr1[$i]} == 1 ]]; then
    color[$i]="(white)"
...
if [[ $column1_background != [1-6] ]]; then
    ...
    echo -e "Column 1 background = ${column1_background} ${color[0]}"
fi

if [[ $column1_font_color != [1-6] ]]; then
    echo -e "Column 1 font color = (default) (black)"
else
    echo -e "Column 1 font color = ${column1_font_color} ${color[1]}"
fi

if [[ $column2_background != [1-6] ]]; then
    echo -e "Column 2 background = (default) (white)"
else
    echo -e "Column 2 background = ${column2_background} ${color[2]}"
fi

if [[ $column2_font_color != [1-6] ]]; then
    echo -e "Column 2 font color = (default) (black)"
else
    echo -e "Column 2 font color = ${column2_font_color} ${color[3]}"
fi
